...
Data_Test = xgb.DMatrix(X, missing=np.NaN)
inference = model.predict(Data_Test)

output_csv = 'output_optimal.csv'
...
with open(output_csv, 'w', newline='') as file:
    ...
    for i in range(len(inference)):
        my_arr.append([id[i], int(inference[i])])
        # writer.writerow([id[i], int(inference[i])])

my_arr.pop(0)
for i, row in enumerate(my_arr):
    if i < 5:
        print(row)
    else:
        break

my_arr.sort(key=lambda x: x[0])
print(my_arr)
